...
class PaymentScreen extends StatefulWidget {
  ...
  const PaymentScreen({Key? key, this.url, this.formCheckout = true}) : super(key: key);
  ...
  void onExit() {
    if(_canRedirect) {
      if(fromCheckout){
        ...
        RouterHelper.getWalletRoute(true, token: '', flag: 'fail', action: RouteAction.pushReplacement);
      }
    }
  ...
  void _pageRedirect(String url) {
    if(_canRedirect) {
      ...
      if(isSuccess){
        ...
        if(isWallet){
           RouterHelper.getWalletRoute(true, token: token, flag: 'success', );
        }else{
          if(token.isNotEmpty) {
         ...
         RouterHelper.getWalletRoute(true, token: 'failed',flag: 'failed', action: RouteAction.pushReplacement);
      } else if(isFailed) {
        RouterHelper.getOrderSuccessScreen('-1', 'payment-fail');
      }else if(isCancel) {
        RouterHelper.getOrderSuccessScreen('-1', 'payment-cancel');
