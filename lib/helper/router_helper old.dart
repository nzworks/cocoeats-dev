...
class RouterHelper {
  ...
  static String getNewPassRoute(String email, String token) => _navigateRoute('$createNewPassScreen?email=$email&token=$token');
  static String getVerifyRoute(String page, String email) {
    String data = Uri.encodeComponent(jsonEncode(email));
    return _navigateRoute('$verify?page=$page&email=$data');
  }

  static String getCreateAccountRoute(String email) {
    String email0 = base64Encode(utf8.encode(email));
    return _navigateRoute('$createAccountScreen?email=$email0');
  }
  static String getMainRoute({RouteAction? action}) => _navigateRoute(dashboardScreen, route: action);
  ...
  static String getCategoryRoute(CategoryModel categoryModel ,{RouteAction? action} ) {
    ...
    return _navigateRoute('$categoryScreen?category=$data', route: action);
  }
  ...
  static String getRateReviewRoute({int? orderId}) => _navigateRoute(rateScreen);
  ...
  static final goRoutes = GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: signupScreen, builder: (context, state) => _routeHandler(context, const SignUpScreen())),
      GoRoute(path: verify, builder: (context, state) => _routeHandler(context, VerificationScreen(
        fromSignUp: state.uri.queryParameters['page'] == 'sign-up', emailAddress: jsonDecode(state.uri.queryParameters['email'] ?? ''),
      ))),
      ...
      GoRoute(path: createNewPassScreen, builder: (context, state) => _routeHandler(context, CreateNewPasswordScreen(
        email: state.uri.queryParameters['email'], resetToken: state.uri.queryParameters['token'],
      ))),
      GoRoute(path: createAccountScreen, builder: (context, state) {
        String? email;
        try{
          email = utf8.decode(base64Decode(state.uri.queryParameters['email']!));
        }catch(error){
          email = null;
        }
        if(email == null) {
      ...
      GoRoute(path: categoryScreen,  builder: (context, state) {
        ...
        try{
          List<int> decode = base64Decode(state.uri.queryParameters['category']!.replaceAll(' ', '+'));
      ...
      GoRoute(path: rateScreen, builder: (context, state) =>  _routeHandler(context, const NotFound())),
      ...
      // GoRoute(path: selectLocationScreen, builder: (context, state) => _routeHandler(context,  const SelectLocationScreen(googleMapController: ,))),
      GoRoute(path: chatScreen, builder: (context, state) {
        final orderModel = jsonDecode(utf8.decode(base64Url.decode('${state.uri.queryParameters['order']?.replaceAll(' ', '+')}')));
